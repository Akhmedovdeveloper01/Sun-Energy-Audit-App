...
exports.handler = async (event) => {
  ...
  try {
    const { chatId, client, extra, arch, eng, devs } = JSON.parse(event.body || '{}');

    console.log('chatId:', chatId);
    console.log('client:', client?.fullName);

    if (!BOT_TOKEN) throw new Error("BOT_TOKEN Netlify Environment Variables ga qo'shilmagan");
    if (!chatId) throw new Error('chatId topilmadi');

    const buffer = await buildDocx({ client, extra, arch, eng, devs });
    console.log('DOCX yaratildi, hajmi:', buffer.length);
    ...
    console.log('Telegram javobi:', tgData);
