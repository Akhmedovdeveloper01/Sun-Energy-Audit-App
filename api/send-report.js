...
async function buildDocx({ client, extra, arch, eng, devs, photos }) {
  ...
  function headerTable() {
    return new Table({
      ...
      rows: [
        ...
        new TableRow({ children: [
          new TableCell({
            borders, rowSpan: 2, width: { size: 5638, type: WidthType.DXA },
            shading: { fill: 'FFFFFF', type: ShadingType.CLEAR },
            margins: { top: 80, bottom: 80, left: 120, right: 120 },
            children: [new Paragraph({ children: [new TextRun({ text: 'Energiya auditorlik xulosasi berilgan', bold: true, size: 20, font: 'Times New Roman' })] })],
          }),
      ...
      rows: [
        ...
        new TableRow({ children: [
          new TableCell({
            ...
            margins: { top: 80, bottom: 80, left: 120, right: 120 },
            children: [new Paragraph({ alignment: AlignmentType.RIGHT, children: [new TextRun({ text: 'Umumiysi:', bold: true, size: 18, font: 'Times New Roman' })] })],
          }),
  ...
  const devsWithPhotos = (devs || []).filter(d => d.photos && d.photos.length > 0);
...
export default async function handler(req, res) {
  ...
  try {
    const { chatId, client, extra, arch, devs = [], photos = [] } = req.body;
