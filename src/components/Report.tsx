...
export default function Report({ data }: ReportProps) {
  ...
  const sendToChat = async () => {
    ...
    try {
      ...
      hapticImpact('heavy');
      showAlert(`✅ Hujjat${photosBase64.length > 0 ? ` va ${photosBase64.length} ta rasm` : ''} chatga yuborildi!`);
    } catch (e: any) {
      showAlert(`❌ Xatolik: ${e.message}`);
    } finally {
      setIsGenerating(false);
        ...
        <div style={sec}>
          ...
          <div style={{ marginBottom: '20px' }}>
            <div style={{ fontSize: '13px', fontWeight: 600, marginBottom: '10px', color: '#333' }}>
              📸 Ob'ekt rasmlari ({photos.length}/10)
            </div>
          ...
          <button onClick={sendToChat} disabled={isGenerating} style={{
            ...
            {isGenerating
              ? '⏳ Yuborilmoqda...'
              : `📨 Yuborish${photos.length > 0 ? ` (${photos.length} rasm + hujjat)` : ''}`}
          </button>

          <p style={{ marginTop: '10px', fontSize: '12px', color: '#888', textAlign: 'center' }}>
            Server .docx yaratadi va rasmlar bilan chatga yuboradi
          </p>
        </div>
      )}
    </div>
